...
class ToolExclusionTests(TestCase):
    def test_excluded_tools_cover_mutating_and_unneeded_tools(self) -> None:
        self.assertEqual(
            EXCLUDED_TOOLS,
            frozenset(
                {
    ...
    def test_create_graph_registers_restrictions_before_agent_creation(
        ...
        build_subagents.assert_called_once()
        self.assertIs(build_subagents.call_args.args[0], sql_tools)
